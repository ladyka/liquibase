...
import liquibase.database.jvm.R2dbcConnection;
...
public class PostgreSQLR2DBCIntegrationTest extends AbstractIntegrationTest {
    ...
    public PostgreSQLR2DBCIntegrationTest() throws Exception {
    ...
    public void testBlobTypesChangeLog() throws Exception {
        ...
        try {
            liquibase.update();
            List<Map<String, ?>>  data = Scope.getCurrentScope().getSingleton(ExecutorService.class).getExecutor("r2dbc", getDatabase())
                    .queryForList(
    ...
    public void testMissingDataGenerator() throws Exception {
        Scope.getCurrentScope().getSingleton(ExecutorService.class).getExecutor("r2dbc", getDatabase())
                  .execute(
    ...
    public void correctlyHandlesAutoIncrementSequences() throws Exception {
        ...
        Scope.getCurrentScope().getSingleton(ExecutorService.class).getExecutor("r2dbc", database)
                .execute(
    ...
    public void validateUserCanOnlyAccessTablesFromSchemasAllowedToRead() throws Exception {
        ...
        ((R2dbcConnection) getDatabase().getConnection()).getUnderlyingConnection().createStatement().executeUpdate(
